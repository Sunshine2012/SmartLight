 ...
 ****************************************************************************************
 */
// used for counting the amount of complete events of writing without response
static uint8_t complete_event_counter_sending_packages = 0; 
                                ...
                                ke_task_id_t const src_id)
{
    ...
    if(state == SPS_CLIENT_DISCOVERING)
    {
			if ((param->status == ATT_ERR_ATTRIBUTE_NOT_FOUND)||
					(param->status == ATT_ERR_NO_ERROR))
				{
						...
						if (!memcmp(sps_client_env->last_svc_req, stream_data_service_uuid, ATT_UUID_128_LEN))
						{
								...
								{
										status = prf_check_svc_char_validity_128(SPS_CLIENT_MAX, sps_client_env->sps_server.chars,
																																							 sps_client_sps_char);
			...
			else if (state == SPS_CLIENT_CONNECTED)
			{
					...
					{
						complete_event_counter_sending_packages++;
					}
			}
...
/**
 ****************************************************************************************
 * @brief Send the write message with no response
 * @param[in] msgid Id of the message received (probably unused).
 * @param[in] param Pointer to the parameters of the message.
 * @param[in] dest_id ID of the receiving task instance (probably unused).
 * @param[in] src_id ID of the sending task instance.
 ...
 ****************************************************************************************
 */
static int sps_client_send_data_packets_req_handler(ke_msg_id_t const msgid,
                                        struct sps_client_send_data_packets_req const *param,
                                        ke_task_id_t const dest_id,
                                        ke_task_id_t const src_id)
{
		struct sps_client_env_tag *sps_client_env = PRF_CLIENT_GET_ENV(dest_id, sps_client);	
		...
		while(packet_counter < param->nr_packets)
		{	
				// get the handle
				uint8_t handle_idx = cb_get_next_tx_att_idx();
				
				// send write with no response
				prf_gatt_write(&sps_client_env->con_info, sps_client_env->sps_server.chars[handle_idx].val_hdl, (uint8_t*) param->packets[packet_counter],param->packets_size[packet_counter],GATTC_WRITE_NO_RESPONSE);	
 ...
 ****************************************************************************************
 */
int sps_client_send_notify_flow_control_state_req_handler(ke_msg_id_t const msgid,
...
const struct ke_msg_handler sps_client_connected[] =
{
		...
		{SPS_CLIENT_SEND_DATA_PACKETS_REQ,		(ke_msg_func_t)sps_client_send_data_packets_req_handler},

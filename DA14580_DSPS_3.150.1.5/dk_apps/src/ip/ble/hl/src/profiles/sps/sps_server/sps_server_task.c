 ...
 ****************************************************************************************
 */
// How many bytes Bluetooth has received
long debug_ble_nb_bytes_received = 0;
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    do 
    {
        status = attmdb_add_service(&(sps_server_env.shdl), TASK_SPS_SERVER,
                                             nb_att_16, nb_att_32, nb_att_128,440); // 16 + (19 + 160 + 2 + 4) + (19 + 1 + 2 + 4) + (19 + 160 + 2 + 4)(185) = 412
																				    // 16 + (19 + 160 + 2 + 14) + (19 + 160 + 2 + 14) + (19 + 1 + 2 +12) = 440
 ...
 ****************************************************************************************
 */
int sps_server_send_notify_flow_control_state_req_handler(ke_msg_id_t const msgid,
...
/**
 ****************************************************************************************
 * @brief Send data over bluetooth @ref sps_send_data_packets_req_handler message.
 * @param[in] msgid Id of the message received 
 * @param[in] param Pointer to the parameters of the message.
 * @param[in] dest_id ID of the receiving task instance (probably unused).
 * @param[in] src_id ID of the sending task instance.
 ...
 ****************************************************************************************
 */
int sps_send_data_packets_req_handler(ke_msg_id_t const msgid,
                                   struct sps_send_data_packets_req const *param,
                                   ke_task_id_t const dest_id,
                                   ke_task_id_t const src_id)
 
{ 
		uint16_t hdl_offset = SPS_SERVER_TX_DATA_VAL - SPS_PRIM_SVC;
	
		// Set value in data base
		attmdb_att_set_value(sps_server_env.shdl + hdl_offset, param->packet_size, (uint8_t*) param->packet);

		// Send notification to the client
		prf_server_send_event((prf_env_struct *)&(sps_server_env.con_info), false, sps_server_env.shdl + hdl_offset);
                                      ...
                                      ke_task_id_t const src_id)
{
		...
		if (param->handle == sps_server_env.shdl + SPS_SERVER_TX_DATA_CFG) 
    {
		...
		{		
			// Increase the test value with the received amount of bytes
			debug_ble_nb_bytes_received += param->length;
			
			// send data to the application
			ble_push((uint8_t*)&(param->value[0]), param->length, 0);
			// 手机端到电脑端
			//ble_push("aiwesky", 7, 0);
			count = (count + 2000) % 16000;
			
			if( ((uint8_t*)&(param->value[0]))[0] == 'a')
			{
			...
			{			
						SetWord16(P1_DATA_REG, (P1_DATA_REG & (~0x01)));
			}
			*/
		}
		...
		{
			updateTransmitAllowed(&ble_flags, param->value[0]);
		}
		else if (param->handle == sps_server_env.shdl + SPS_FLOW_CTRL_CFG)
		{
			if (param->value[0]==0x01 && param->value[1]==0x00)
			{
				sendFlowControlOverBluetooth((last_ble_flag==1 ? UART_XON : UART_XOFF));
			}
			if (param->response==0x01)
			{
				atts_write_rsp_send(sps_server_env.con_info.conidx, param->handle, PRF_ERR_OK);				
...
const struct ke_msg_handler sps_server_active[] =
{
		...
		{SPS_SEND_DATA_PACKETS_REQ,					(ke_msg_func_t)sps_send_data_packets_req_handler}
}; 

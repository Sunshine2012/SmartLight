 ...
 *
 * @brief 16 UUID service. sample code
 *
                                       ...
                                       ke_task_id_t const src_id)
{
     ...
     *---------------------------------------------------*/
        ...
        nb_att_16 = 3;  // 4 UUID16 atts
        nb_att_32 = 0;  // No UUID32 att
        nb_att_128 = 0; //No UUID128 att
        status = attmdb_add_service(&(wechat_env.wechat_shdl), TASK_WECHAT,
                                             nb_att_16, nb_att_32, nb_att_128, 50); //total attributte size = 36, 16 (svc)  + 19 (desc_char) + 1 (att)

        if (status == ATT_ERR_NO_ERROR)
        {
            //add svc attribute
            status = attmdb_add_attribute(wechat_env.wechat_shdl, ATT_UUID_16_LEN, 
                                               ATT_UUID_16_LEN, (uint8_t*)&att_decl_svc, PERM(RD, ENABLE),
                                               &(wechat_env.wechat_shdl));
            
            
            
            status = attmdb_att_set_value(wechat_env.wechat_shdl, ATT_UUID_16_LEN, (uint8_t *)wechat_svc);
            
            
//char 1     
            //add char attribute
            status = attmdb_add_attribute(wechat_env.wechat_shdl, ATT_UUID_16_LEN, //Data size = 19 (ATT_UUID_128_LEN + 3)
            ...
            //add val attribute
            status = attmdb_add_attribute(wechat_env.wechat_shdl, sizeof(uint8_t), //Data size = 1
                                               ATT_UUID_16_LEN, (uint8_t*)&wechat_1_val.uuid, PERM(RD, ENABLE) | PERM(WR, ENABLE),
                                               &(val_hdl));
            
            memcpy(wechat_1_char.attr_hdl, &val_hdl, sizeof(uint16_t));
            
            status = attmdb_att_set_value(char_hdl, sizeof(wechat_1_char), (uint8_t *)&wechat_1_char);
...
//////char 2
            ...
            //add val attribute
            status = attmdb_add_attribute(wechat_env.wechat_shdl, sizeof(uint8_t), //Data size = 1
                                               ATT_UUID_16_LEN, (uint8_t*)&wechat_2_val.uuid, PERM(RD, ENABLE) | PERM(NTF, ENABLE),
                                               &(val_hdl));
            
            memcpy(wechat_2_char.attr_hdl, &val_hdl, sizeof(uint16_t));
            
            status = attmdb_att_set_value(char_hdl, sizeof(wechat_2_char), (uint8_t *)&wechat_2_char);

            //add cfg attribute
            status = attmdb_add_attribute(wechat_env.wechat_shdl, sizeof(uint16_t), 
                                               ATT_UUID_16_LEN, (uint8_t*) &att_decl_cfg, PERM(RD, ENABLE) | PERM(WR, ENABLE),
                                               &(val_hdl));

            //Disable wechat service
           ...
           ke_state_set(TASK_WECHAT, SPS_SERVER_ACTIVE);
        }
                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        attmdb_att_set_value(wechat_env.wechat_shdl + WECHAT_1_IDX_VAL,
                             sizeof(uint8_t), (uint8_t *)&param->wechat_1_val);
        
        attmdb_att_set_value(wechat_env.wechat_shdl + WECHAT_2_IDX_VAL,
                             sizeof(uint8_t), (uint8_t *)&param->wechat_2_val);
        ...
        attmdb_att_set_value(wechat_env.wechat_shdl + WECHAT_2_IDX_CFG,
...
static int wechat_upd_char2_req_handler(ke_msg_id_t const msgid,
                                    struct wechat_upd_char2_req const *param,
                                    ke_task_id_t const dest_id,
                                    ke_task_id_t const src_id)
{
    ...
    if(param->conhdl == gapc_get_conhdl(wechat_env.con_info.conidx))
    {
        ...
        if((wechat_env.feature & PRF_CLI_START_NTF))
            // Send notification through GATT
            prf_server_send_event((prf_env_struct *)&wechat_env, false,
                    wechat_env.wechat_shdl + WECHAT_2_IDX_VAL);
    ...
    {
        wechat_upd_char2_cfm_send(status);
    }
...
static int gattc_write_cmd_ind_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    uint8_t char_code = WECHAT_ERR_CHAR;
    uint8_t status = PRF_APP_ERROR;

    if (KE_IDX_GET(src_id) == wechat_env.con_info.conidx)
    {
        if (param->handle == wechat_env.wechat_shdl + WECHAT_1_IDX_VAL)
        {
            char_code = WECHAT_1_CHAR;
        }
        
        if (param->handle == wechat_env.wechat_shdl + WECHAT_2_IDX_CFG)
        {
            char_code = WECHAT_2_CFG;
        }
        
        if (char_code == WECHAT_1_CHAR)
        {
            ...
            if(param->last)
            {
                wechat_send_val(param->value[0]);
            }

            status = PRF_ERR_OK;
               
        }
        else if (char_code == WECHAT_2_CFG)
        {
            ...
            if ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF))
            {
                ...
                if (ntf_cfg == PRF_CLI_START_NTF)
                {
                    ...
                    wechat_env.feature |= PRF_CLI_START_NTF;
                }
                else
                {
...
const struct ke_msg_handler wechat_connected[] =
{
    ...
    {WECHAT_UPD_CHAR2_REQ,   (ke_msg_func_t) wechat_upd_char2_req_handler},
};
